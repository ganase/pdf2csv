...
以下の2つのファイルをダウンロードして、同じフォルダ（例：`C:\作業\PDF2CSV\`）に保存します。

| ファイル | 役割 |
|---------|------|
| **[📥 setup.bat をダウンロード](https://raw.githubusercontent.com/ganase/pdf2csv/main/setup.bat)** | 最初に実行するファイル |
| **[📥 installer.py をダウンロード](https://raw.githubusercontent.com/ganase/pdf2csv/main/installer.py)** | セットアップ画面の本体 |

> **ダウンロード方法：** リンクを右クリック →「名前を付けてリンク先を保存」

### ステップ 2：setup.bat をダブルクリックする
...
自動でブラウザが開くので、Pythonをインストールしてから `setup.bat` を再実行してください。
